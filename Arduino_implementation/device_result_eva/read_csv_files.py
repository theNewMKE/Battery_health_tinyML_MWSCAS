...
res_path = "../device_result"
y1_pred_micro_TCN = pd.read_csv(res_path+"/results_TCN_tar1_revised.csv")
y2_pred_micro_TCN = pd.read_csv(res_path+"/results_TCN_tar2_revised.csv")


y1_pred_micro_CNN = pd.read_csv(res_path+"/results_CNN_tar1_revised.csv")
y2_pred_micro_CNN = pd.read_csv(res_path+"/results_CNN_tar2_revised.csv")
...
plt.plot(y1_pred_micro_TCN_series.sort_index(), 'ro', markersize=8, alpha=0.5, label='TCN')
...
# plt.savefig('RUL_vs_TS_micro.pdf')
plt.show()
...
plt.plot(y2_pred_micro_TCN_series.sort_index(), 'ro', markersize=8, alpha=0.5, label='TCN')
# print(y_pred_series.sort_index())
plt.xlabel('Time step (10 seconds)')
plt.ylabel('Cell temperature (°C)')
plt.legend(loc='upper left')
# plt.savefig('Temp_vs_TS_micro.pdf')
plt.show()
...
def cal_error(y_test_reshaped: np.ndarray, y_pred_test: np.ndarray):
    ...
    # calculate MAE
    mae = round(mean_absolute_error(y_test_reshaped, y_pred_test), 3)

    # calculate MSE
    mse = round(mean_squared_error(y_test_reshaped, y_pred_test), 3)

    # calculate RMSE
    rmse = round(np.sqrt(mean_squared_error(y_test_reshaped, y_pred_test)), 3)

    # Calculate R2
    r2 = round(r2_score(y_test_reshaped, y_pred_test), 3)
